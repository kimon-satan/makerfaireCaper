...
function sketchProc(p) {
	...
	p.setup = function(){
		...
		for(var i = 0; i < 15; i++){
			...
			for(var j = 0; j < 15; j++){
					...
					tLinks.push((Math.random() > 0.5) ? Math.round(Math.min(Math.random() * 500,Math.random() * 500)) : 0);
		...
		for(var i = 0; i < 5; i++){
			...
			var pop = Math.random() * 300;
			var dim = Math.max(100, pop);
  ...
  drawCylinder = function( sides, r, h){
	...
	p.draw = function() {
		...
		var orbitRadius = p.width/4 + 3000;
		var ypos= 0;
